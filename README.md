...
**LightNote** 是一个轻量、离线优先的桌面 Markdown 编辑器。它使用 Tauri 2 构建，单栏模式专注于源码编辑，分栏模式提供右侧实时渲染预览，并支持**多文档标签页**同时打开多个文件。
...
- 界面语言：在“视图 -> 语言”中选择跟随系统、中文简体或 English；选择后立即生效，并会记住选择。
...
- 多标签页：可同时打开多个文档，每个文档对应一个标签，切换标签时保留各自的未保存内容与光标位置。
- 标签自动去重：再次打开已在其他标签中的文件会自动切换到已有标签，避免重复编辑同一文件。
- 未保存保护：关闭有未保存修改的标签时提示确认，避免误丢内容；未做任何改动的标签可直接关闭。
- 离线优先：编辑与本地保存不依赖远程服务。
...
| 前端 | Vite、Vanilla TypeScript | 视图切换、多标签页管理、文件导入保存和菜单交互。 |
...
- “打开文件”：导入 `.md`、`.markdown` 或 `.txt`，每次打开生成一个标签页；再次打开已在其他标签中的文件会自动切换过去。
- 标签页：点击顶部标签可切换文档（`Ctrl+Tab` / `Ctrl+Shift+Tab` 循环），「＋」按钮或 `Ctrl+N` 新建空白标签，标签上的「×」或 `Ctrl+W` 关闭当前标签。切换或关闭时保留各标签未保存内容；关闭有未保存修改的标签会先提示确认。
- “保存文件”：将当前内容写回已打开的可写文件；只读文件或新文档会弹出保存位置。也可以使用 `Ctrl+S`。
- “视图 -> 主题”：可选择跟随系统、浅色或深色主题，页面、编辑器、预览、代码块和 Mermaid 图表会同步更新。
- “视图 -> 语言”：可选择跟随系统、中文简体或 English。默认跟随操作系统语言；手动选择后，菜单、窗口标题和应用文案会立即统一切换。Windows 原生打开/保存对话框继续跟随 Windows 系统语言。
...
| `Ctrl+N` | 当前窗口 | 新建标签 |
| `Ctrl+W` | 当前标签 | 关闭当前标签 |
| `Ctrl+Tab` / `Ctrl+Shift+Tab` | 当前窗口 | 下一个/上一个标签 |
...
.
├── src/
│   ├── tabs.ts           # 多标签页状态模型（TabManager）与标签栏逻辑
│   ├── main.ts           # 编辑器、预览、文件操作与标签页联动
│   ├── styles.css        # 应用与编辑器样式
│   ├── types.d.ts        # 前端类型声明
│   └── assets/           # 前端静态资源
├── src-tauri/
...
LightNote 通过多标签同时处理多个打开的文件，但不扫描目录，不维护文档库或全文索引，也不会在应用数据目录保存文档副本。已打开的可写文件会自动写回；未指定保存位置的新文档在应用关闭后不会保留。

## 已知限制

- 标签页仅保留各文档的文本缓冲，切换标签后该标签的撤销历史不会保留。
